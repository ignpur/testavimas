...
{
    [TestFixture]
    internal class MixedShipPlacementTests
    {
        private PlayerOne _player;
        [SetUp]
        public void Setup()
        {
            _player = new PlayerOne("TestPlayer", new MixedShipPlacementStrategy());
        ...
        public void TestValidShipPlacement()
        {
            ...
            if (result.vertical)
            {
                for (int i = result.posY; i < result.posY + size; i++)
                {
            ...
            {
                for (int i = result.posX; i < result.posX + size; i++)
                {
        ...
        public void TestShipPlacement_InvalidCoordinates_ShouldPlaceInEmptySpot()
        {
            ...
            // Assert
            Assert.IsTrue(result.result, "The ship placement should fail due to out-of-bounds placement.");
        }
        ...
        public void TestShipPlacement_Overlap_ShouldPlaceShipInEmptySpot()
        {
            ...
            // Assert
            Assert.IsTrue(result.result, "The ship placement should fail due to overlap.");
        }
    }
}
